...
//inicio adriano
  bool Inside_Interest_Area(Vector origem_interesse,
		  	  	  	  	  	Vector destino_interesse,
		  	  	  	  	  	Vector posicao_no);

  //funcao simplificada da CalculaRetanguloInteresse
  Rectangle RetanguloInteresse(Vector o_i,
  		  	  	  	  	  	   Vector d_i,
  		  	  	  	  	  	   int coef_linear);


  Vector3D GetTagOrigem(Ptr<const Packet> origPacket);

  Vector3D GetTagDestino(Ptr<const InterestHeader> header);

  //FUNCAO QUE NECESSITA DE CORRECOES...
  //Rectangle CalculaRetanguloInteresse(Vector2D o_i,
  //		  	  	  	  	  	   	   	  Vector2D d_i,
  //		  	  	  	  	  	   	   	  int coef_linear);
