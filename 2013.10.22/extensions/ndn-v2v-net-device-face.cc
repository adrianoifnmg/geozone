...
V2vNetDeviceFace::SendImpl (Ptr<Packet> packet)
{
  ...
  if (type == HeaderHelper::INTEREST_CCNB ||
      type == HeaderHelper::INTEREST_NDNSIM)
    {
      ...
      //////////////////////////////
	  NS_LOG_INFO ("SendImpl -> Envio Interesse pelo No: "<< m_node->GetId());
  ...
  if (m_queue.size () > 0)
    {
      ...
      if (item.m_type == HeaderHelper::INTEREST_NDNSIM)
        {
    	  NS_LOG_INFO ("TagAndNetDeviceSendImpl (Queue) -> Envio Interesse pelo No: "<< m_node->GetId());
          m_txInterest (m_node, item.m_packet, mobility->GetPosition ());
        }
      else
        {
    	  NS_LOG_INFO ("TagAndNetDeviceSendImpl (Queue) -> Envio Conteudo pelo No: "<< m_node->GetId());
  ...
  else if (m_lowPriorityQueue.size () > 0) // no reason for this check, just for readability
    {
      ...
      if (item.m_type == HeaderHelper::INTEREST_NDNSIM)
        {
    	  NS_LOG_INFO ("TagAndNetDeviceSendImpl (LowQueue) -> Envio Interesse pelo No: "<< m_node->GetId());
          m_txInterest (m_node, item.m_packet, mobility->GetPosition ());
        }
      else
        {
    	  NS_LOG_INFO ("TagAndNetDeviceSendImpl (LowQueue) -> Envio Conteudo pelo No: "<< m_node->GetId());
          m_txData (m_node, item.m_packet, mobility->GetPosition ());
        }
      //////////////////////////////
  ...
  if (mobility != 0)
    {
      ...
      //NS_LOG_INFO ("Retransmissao do No: " << m_node->GetId());
                                        ...
                                        NetDevice::PacketType)
{
  ...
  if (packetType==HeaderHelper::CONTENT_OBJECT_NDNSIM)
	  NS_LOG_DEBUG ("SANITY CHECK: CONTEUDO: "<< name->GetComponents()<< " " << isSrcTag << ", " << isTransmissionTag << ", " << mobility);
  else
	  NS_LOG_DEBUG ("SANITY CHECK: INTERESSE: "<< name->GetComponents()<< " " << isSrcTag << ", " << isTransmissionTag << ", " << mobility);

  //   src  -----   <transmission>  ---- <mobility>
  bool needToCancel = true;
  if (mobility && isSrcTag && isTransmissionTag)
    {
      NS_LOG_DEBUG ("Check distances:" << CalculateDistance (srcTag.GetPosition (), transmissionTag.GetPosition ()) << " <? " << CalculateDistance (srcTag.GetPosition (), mobility->GetPosition ()) << " Yes? OK!");
      if (CalculateDistance (srcTag.GetPosition (), transmissionTag.GetPosition ())
          ...
          CalculateDistance (srcTag.GetPosition (), mobility->GetPosition ()))
        {
          ...
          NS_LOG_DEBUG ("Cancel in Queues = True! Receptor esta entre o Relay e Source");
      ...
      if ((packetType==HeaderHelper::CONTENT_OBJECT_NDNSIM || item->m_type == packetType) && *item->m_name == *name)
        {
          ...
          if (cancelled)
        	 NS_LOG_DEBUG ("Cancelled Reception "<< name->GetComponents() <<" ja esta na Fila LowPriority!");
          ...
          if (needToCancel)
            {
              ...
              if (packetType == HeaderHelper::CONTENT_OBJECT_NDNSIM)
            	  NS_LOG_INFO ("Canceling Content Object with name " << name->GetComponents() << ", which is scheduled for low-priority transmission");
              else
            	  NS_LOG_INFO ("Canceling Interest Object with name " << name->GetComponents() << ", which is scheduled for low-priority transmission");
      ...
      if ((packetType==HeaderHelper::CONTENT_OBJECT_NDNSIM || item->m_type == packetType) && *item->m_name == *name)
        {
          cancelled = item->m_type == packetType;
          if (cancelled)
        	 NS_LOG_DEBUG ("Cancelled Reception "<< name->GetComponents() <<" ja esta na Fila Normal!");
          ...
          if (needToCancel)
            {
              ...
              if (packetType == HeaderHelper::CONTENT_OBJECT_NDNSIM)
            	  NS_LOG_INFO ("Canceling Content Object with name " << name->GetComponents() << ", which is scheduled for normal-priority transmission");
              else
            	  NS_LOG_INFO ("Canceling Interest Object with name " << name->GetComponents() << ", which is scheduled for normal-priority transmission");
      ...
      if ((packetType==HeaderHelper::CONTENT_OBJECT_NDNSIM || item->m_type == packetType) && *item->m_name == *name)
        {
          cancelled = item->m_type == packetType;
          if (cancelled)
        	 NS_LOG_DEBUG ("Cancelled Reception "<< name->GetComponents() <<" ja esta na Fila de Retransmissao!");
          ...
          if (needToCancel)
            {
              ...
              if (packetType == HeaderHelper::CONTENT_OBJECT_NDNSIM)
            	  NS_LOG_INFO ("Canceling Content Object with name " << name->GetComponents() << ", which is scheduled for retransmission");
              else
            	  NS_LOG_INFO ("Canceling Interest Object with name " << name->GetComponents() << ", which is scheduled for retransmission");
  ...
  if (cancelled)
    {
      ...
      //  }
      //NS_LOG_DEBUG ("Cancelled Reception "<< name <<" ja esta na Fila!");
      return;
    }
  else{
